...
st.title("🚨 Hệ thống cảnh báo khách thuê xe (DeepFace + SFace, chạy trên Streamlit Cloud)")
...
def check_image(img_file):
    ...
    for file in os.listdir(DATA_DIR):
        if file.endswith(".jpg"):
            ...
            try:
                # Dùng model nhẹ "SFace"
                result = DeepFace.verify(new_img_path, db_img_path, model_name="SFace", enforce_detection=False)
                if result["verified"]:
